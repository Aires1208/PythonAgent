...
class _ObjectProxyMethods(object):
    ...
    # We use properties to override the values of __module__ and __doc__.
    # If we add these in ObjectProxy,
    # the derived class __dict__ will still be setup to have string variants of these attributes
    # and
    # the rules of descriptors means that they appear to take precedence over the properties in the base class.
    # To avoid that, we copy the properties into the derived class type itself via a meta class.
    # In that way the properties will always take precedence.
    ...
    # We similar use a property for __dict__.
    # We need __dict__ to be explicit to ensure that vars() works as expected.

    @property
    def __dict__(self):
        return self.__wrapped__.__dict__

    # Need to also propagate the special __weakref__ attribute for case where decorating classes which will define this.
    # If do not define it and use a function like inspect.getmembers() on a decorator class it will fail.
    # This can't be in the derived classes.
...
class _ObjectProxyMetaType(type):
    def __new__(cls, name, bases, dictionary):
        ...
        # Copy our special properties into the class
        # so that they always take precedence over attributes of the same name added during construction of a derived class.
        # This is to save duplicating the implementation for them in all derived classes.
...
class ObjectProxy(with_metaclass(_ObjectProxyMetaType)):
    ...
    def __init__(self, wrapped):
        ...
        # Python 3.2+ has the __qualname__ attribute,
        # but it does not allow it to be overridden using a property
        # and it must instead be an actual string object instead.
