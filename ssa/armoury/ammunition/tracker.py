...
class Tracker(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb, async=False):
        ...
        Zipkin(environ=self.environ, framework=self.framework).handle(node)
    ...
    def process_database_node(self, node):
        ...
        print("node.duration:%s" % node.duration)
        print("slow_sql_threshold:%s" % self._settings.action_tracer.slow_sql_threshold)
