...
from ssa.logistics.mapper import CONSTANCE_OUT_DATE_CONFIG, CONSTANCE_INVALID_DATA_TOKEN
from ssa.logistics.mapper import CONSTANCE_INVALID_LICENSE_KEY
from ssa.armoury.sampler.memory_usage import memory_usage_sampler
from ssa.armoury.sampler.cpu_usage import cpu_usage_sampler
from ssa.battlefield.application import Application
...
class Dispatcher(object):
    ...
    def _do_harvest(self, last_harvest, current_harvest, shutdown=False):
        ...
        for name, application in six.iteritems(self._applications):
            print("name:%s,application:%s" % (name, application))
        ...
        with self._lock:
            ...
            if not app:
                try:
                    ...
                    print("app_name:%s,app:%s" % (app_name, app))
